...
export default function RegionModal({ isOpen, onClose }: RegionModalProps) {
  ...
  const handleZipCodeSubmit = () => {
    try {
      ...
      if (!/^\d{5}$/.test(zipCode)) {
        setErrors({ zip_code: "Please enter a valid 5-digit ZIP code" })
        setTouched({ zip_code: true })
        return
      }

      // Find matching region
      const matchingRegion = regions.find(r => r.zipCodes.includes(zipCode))
      if (!matchingRegion) {
        setErrors({ zip_code: "Sorry, we don't service this area yet" })
        setTouched({ zip_code: true })
        return
      }

      // Update region and close modal
      setRegionByZipCode(zipCode)
      setZipCode("") // Reset input
      onClose()
    } catch (error) {
      console.error("Error submitting ZIP code:", error)
      setErrors({ zip_code: "An error occurred. Please try again." })
      setTouched({ zip_code: true })
    } finally {
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    {regions.map((region) => (
                      <button 
                        key={region.name}
                        className="flex flex-col items-center p-4 border rounded-lg cursor-pointer hover:border-orange-500 hover:bg-orange-50 transition-colors w-full"
                        onClick={() => handleRegionSelect(region.name)}
                        aria-label={`Select ${region.name} as your service area`}
                      >
